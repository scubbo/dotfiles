...
def main(args):
  ...
  for p in git_dirs:
    branches = {branch_name.stem: get_commit_info(p, branch_name) for branch_name in p.joinpath('.git', 'refs', 'heads').iterdir()}
    print(p)
    print(branches)
    # Just testing!
    # TODO next:
...
def get_commit_info(path, branch_name):
  ...
  try:
    ...
    offset_in_packfile = find_packfile_offset_for_commit(path, commit_id)
    commit_info_raw = extract_commit_info_from_packfile_as_offset(path, offset_in_packfile)
...
def extract_commit_info_from_packfile_as_offset(repo_dir, offset):
  ...
  with open(pack_file_path.absolute(), 'rb') as f:
    ...
    print(f'DEBUG - obj_type is {obj_type}')
...
def find_packfile_offset_for_commit(repo_dir, commit_sha):
...
# Not read_bytes() because we can optimize (lol, as if that makes a difference!) by doing seeks
  with open(idx_path.absolute(), 'rb') as f:
    ...
    for i in range(num_prefix_objects):
      ...
      raise Exception(f'Could not find commit {commit_sha} within expected number of objects')
